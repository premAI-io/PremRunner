...
const chunkedUpload = new Hono()
  .post("/start", async (c) => {
    ...
    console.log(
      `📊 Total size: ${(fileSize / 1024 / 1024 / 1024).toFixed(2)} GB`,
    );
  ...
  .post("/chunk", async (c) => {
    ...
    console.log(
      `📦 Saved chunk ${chunkIndex + 1}/${totalChunks} for model ${modelId}`,
    );
  ...
  .post("/complete", async (c) => {
    ...
    for (let i = 0; i < totalChunks; i++) {
      ...
      console.log(
        `📦 Writing chunk ${i + 1}/${totalChunks}, size: ${chunkSize}`,
      );
    ...
    await writer.end();
    console.log(
      `✅ Assembled ${totalChunks} chunks into ${(totalSize / 1024 / 1024 / 1024).toFixed(2)} GB file`,
    );
    ...
    console.log(
      `📊 Final size: ${(finalSize / 1024 / 1024 / 1024).toFixed(2)} GB`,
    );
