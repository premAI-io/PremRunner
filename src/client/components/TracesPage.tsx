        ...
        <div className="max-w-6xl mx-auto">
          ...
          {loading ? (
            ...
            <div className="bg-stone-50 rounded-lg border border-stone-200 p-12 text-center">
              ...
              <p className="text-lg font-medium text-stone-950 mb-1">No traces yet</p>
              <p className="text-sm text-stone-500">Start chatting with your models to see traces here</p>
            </div>
          ) : (
            <>
                      ...
                      <tr key={trace.id} className="hover:bg-stone-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-stone-900">
                          <div className="flex items-center gap-2">
                            <span className="text-xs text-stone-500">In:</span> {trace.promptTokens}
                            <span className="text-xs text-stone-500">Out:</span> {trace.completionTokens}
                          </div>
                        </td>
              ...
              {totalPages > 1 && (
                <div className="mt-6 flex items-center justify-between">
                  <p className="text-sm text-stone-500">
                    Showing {((page - 1) * 20) + 1} to {Math.min(page * 20, total)} of {total} traces
                  </p>
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[80vh] overflow-hidden flex flex-col">
            <div className="px-6 py-4 border-b border-stone-200">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-semibold text-stone-950">Trace Details</h3>
                <button
            ...
            <div className="flex-1 overflow-y-auto p-6">
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-sm font-medium text-stone-500 mb-1">Model</p>
                    <p className="text-sm text-stone-950">{selectedTrace.model}</p>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-stone-500 mb-1">Duration</p>
                    <p className="text-sm text-stone-950">{formatDuration(selectedTrace.duration)}</p>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-stone-500 mb-1">Total Tokens</p>
                    <p className="text-sm text-stone-950">{selectedTrace.totalTokens}</p>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-stone-500 mb-1">Timestamp</p>
                    <p className="text-sm text-stone-950">{formatDate(selectedTrace.createdAt)}</p>
                  </div>
                </div>

                {/* Input */}
                <div>
                  <p className="text-sm font-medium text-stone-700 mb-2">Input Conversation</p>
                ...
                {/* Output */}
                <div>
                  <p className="text-sm font-medium text-stone-700 mb-2">Model Response</p>
                  <div className="bg-stone-50 rounded-lg p-4 border border-stone-200">
                    <pre className="text-sm text-stone-950 whitespace-pre-wrap">
