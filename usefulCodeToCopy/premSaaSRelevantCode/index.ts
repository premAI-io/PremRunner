...
import {
  updateFineTuningTask,
  updateInferenceTask,
} from "./api/finetuning/utils";
...
const app = new Hono()
  ...
  .use(
    "/api/v1/finetuning/create",
    requestQuotaLimiter("fineTuningExperiments"),
  )
...
app
  ...
  .use(
    ...
    internalChatMiddleware,
    requestQuotaLimiter("llmRequests"),
  )
...
app
  .use("/api/webhooks/stripe", cors(), logger())
  .route("/api/webhooks/stripe", stripeApp);

// BitGPT routes (no authentication required for webhooks)
app
  .use("/api/webhooks/bitgpt", cors(), logger())
  .route("/api/webhooks/bitgpt", bitgptApp);
...
CronJob.from({
  ...
  onTick: function () {
    console.log(
      `${new Date().toISOString()} Running data augmentation cleanup`,
    );
