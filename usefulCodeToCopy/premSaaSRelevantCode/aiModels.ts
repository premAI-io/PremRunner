...
export async function getPremInferenceModels(userId: string): Promise<ChatModel[][]> {
...
export async function resolveModelByIdOrAlias(
	...
	const openAiIndex = openAIModels.findIndex((m) => m.alias === idOrAlias || m.id === idOrAlias);
	if (openAiIndex !== -1) {
		return openAIModels[openAiIndex];
	}

	const bedrockIndex = bedrockModels.findIndex((m) => m.alias === idOrAlias || m.id === idOrAlias);
	if (bedrockIndex !== -1) {
	...
	const premIndex = premModels.findIndex((m) => m.alias === idOrAlias || m.id === idOrAlias);
	if (premIndex !== -1) {
...
const slugs = [...openAIModels, ...bedrockModels, ...openRouterModels].map((m) => m.alias);
